...
/**
 * 
 * La classe MossaStato rappresenta una mossa di stato nei combattimenti
 * Pokémon.
 * Le mosse di stato non infliggono danno, ma modificano le caratteristiche dei
...
public class MossaStato extends Mossa {
    ...
    public MossaStato(String nomeMossa, int elementoMossa, String tipo, int PP, int PPmax, int precisioneMossa,
            String effetto) {
    ...
    /**
     * Metodo per applicare l'effetto della mossa di stato, modificando le
    ...
    public void usaMossaStato(Pokemon attaccante, Pokemon difensore) {
        ...
        switch (effetto) {
            ...
            case "Diminuisce Difesa":
                bersaglio.setDifesa((int) Math.max(1, bersaglio.getDifesa()
                        * (effetto.startsWith("Aumenta") ? 1 + valoreModifica : 1 - valoreModifica)));
                break;
            case "Aumenta Attacco":
            case "Diminuisce Attacco":
                bersaglio.setAttacco((int) Math.max(1, bersaglio.getAttacco()
                        * (effetto.startsWith("Aumenta") ? 1 + valoreModifica : 1 - valoreModifica)));
                break;
            case "Aumenta Difesa Speciale":
            case "Diminuisce Difesa Speciale":
                bersaglio.setDifesaSpeciale((int) Math.max(1, bersaglio.getDifesaSpeciale()
                        * (effetto.startsWith("Aumenta") ? 1 + valoreModifica : 1 - valoreModifica)));
                break;
            case "Aumenta Attacco Speciale":
            case "Diminuisce Attacco Speciale":
                bersaglio.setAttaccoSpeciale((int) Math.max(1, bersaglio.getAttaccoSpeciale()
                        * (effetto.startsWith("Aumenta") ? 1 + valoreModifica : 1 - valoreModifica)));
                break;
            case "Aumenta Velocità":
            case "Diminuisce Velocità":
                bersaglio.setVelocita((int) Math.max(1, bersaglio.getVelocita()
                        * (effetto.startsWith("Aumenta") ? 1 + valoreModifica : 1 - valoreModifica)));
                break;
            case "Aumenta Elusione":
            case "Diminuisce Elusione":
                bersaglio.setElusione((int) Math.max(1, bersaglio.getElusione()
                        * (effetto.startsWith("Aumenta") ? 1 + valoreModifica : 1 - valoreModifica)));
                break;
        }
    }
